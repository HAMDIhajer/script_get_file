...
case "$MACHINE" in
    ...
    *)
     echo "Impossible de determiner la bande de test AP" | tee $LOG_FILE
     exit 1
esac
...
if [ $file_number == 0 ]
then
	echo "Le repertoire data de la machine cleinte ne contient aucun fichier csv" | tee $LOG_FILE
        exit 0
fi	
